...
public class ShipScript : MonoBehaviour
{
	...
	void Update ()
	{
		...
		distance = Vector3.Distance (transform.position, FindClosestShip().transform.position);
		if (distance < 6) closeEnough = true; //we are close to an island
		if (distance > 7) closeEnough = false;


		//distance checker if enemies near
		//check if enemies
		...
		if (GameObject.FindGameObjectWithTag("enemy") != null){ 
			...
			if (distance > 10) enemyNear = false;
			
			if (enemyNear) {
				Debug.Log ("WE ARE CLOSE TO AN ENEMY");
				chaseShip(FindClosestEnemy());
			} else Debug.Log ("WE ARENT CLOSE TO ENEMY");
		} else Debug.Log ("enemy doesnt exist");
